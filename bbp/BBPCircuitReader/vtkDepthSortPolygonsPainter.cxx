/*=========================================================================
  ...
  Module:    vtkDepthSortPolygonsPainter.cxx
...
{ \
  ...
  glInitFuncs \
  while (cellId < totalCells) \
    { \
    vtkIdType *_tuple = &orderedDepthInfo[cellId*2]; \
    vtkIdType  sortedId = _tuple[0]; \
    vtkIdType  nPts     =  cellArrayData[_tuple[1]]; \
    vtkIdType *ptIds    = &cellArrayData[_tuple[1]+1]; \
    ...
    vtkUnsignedCharArray* c, vtkDataArray* t, vtkRenderer* ren)
{
  ...
  if (cellNum>0 || !DepthOrder) {
    return this->Superclass::RenderPrimitive(idx, n,c, t, ren);
  }
  //
  vtkPoints     *p = pd->GetPoints();
  vtkCellArray *ca = pd->GetPolys();
  vtkIdType    *cellArrayData = ca->GetPointer();
  vtkIdType cellNumStart = 0;
  vtkIdType *orderedDepthInfo, cellId = 0;
  vtkIdType totalCells = ca->GetNumberOfCells();
  vtkUnsignedCharArray *ef = vtkUnsignedCharArray::SafeDownCast(pd->GetPointData()->GetAttribute(vtkDataSetAttributes::EDGEFLAG));

  vtkPainterDeviceAdapter* device = ren->GetRenderWindow()->GetPainterDeviceAdapter();
  ...
  if (DepthOrder) 
  {
    orderedDepthInfo = DepthOrder->GetPointer(0);
  }
  if (n)
    {
